    ...
    $("#form-set-submit").click(function () {
        ...
        if (token.length > 0) {
            if (token.length < TOKEN_MIN_LENGTH || token.length > TOKEN_MAX_LENGTH) {
                $("#modal-msg-content").html("Url custom harus " + TOKEN_MIN_LENGTH + " - " + TOKEN_MAX_LENGTH + " Character");
            ...
            if (!pattern.test(token)) {
                $("#modal-msg-content").html("Url custom tidak valid, hanya mendukung huruf dan angka");
                $("#modal-msg").modal('show');
                return true;
            }
        }
        if (url.length < URL_MIN_LENGTH || url.length > URL_MAX_LENGTH) {
            $("#modal-msg-content").html("Panjang Url dalam " + URL_MIN_LENGTH + " - " + URL_MAX_LENGTH);
            $("#modal-msg").modal('show');
            return true;
        }
...
function show_result(url, shortUrl) {
    ...
    $("#modal-result-title").text("URL pendek dihasilkan");
